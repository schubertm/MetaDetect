...
def make_predictions(df):
    ...
    for i in range(len(all_image_paths)):
        ...
        #print(df_temp)
        #print(ads)
        #array = np.full((len(bboxes_single_image)),df.loc[zaehler,'file_path'])
        #df_single_image = np.concatenate((np.asmatrix(np.full((len(bboxes_single_image)),df.loc[zaehler,'file_path'])).T,np.asmatrix(bboxes_single_image)),axis=1)
        try:
...
def calculate_map(df, gt_path):
    ...
    #df = load_single_frame('/home/riedlinger/MetaDetect-Merged/metrics_cat.csv')
    #df = load_single_frame('/home/riedlinger/MetaDetect-TestEvaluation/metrics_cat.csv')

    df = df.loc[:, 'file_path':'category_idx']
    df["s"] = pd.to_numeric(df["s"], downcast="float")
    # df = df.loc[ df['s'] >= 0.000001 ]
    ...
    # number_of_gt = np.zeros(num_classes)
    for c in range(num_classes):
        ...
        # plt.savefig('/home/schubert/plot.png')
        # print(asd)
    print(mean_ap)
...
if __name__ == '__main__': 
    ...
    df = load_single_frame('/home/riedlinger/MetaDetect-TestEvaluation/test_marius/metrics_cat.csv')
